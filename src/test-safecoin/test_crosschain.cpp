...
extern uint256 safecoin_calcMoM(int32_t height,int32_t MoMdepth);
extern bool SAFECOIN_TEST_ASSETCHAIN_SKIP_POW;
...
TEST_F(TestCrossChain, testCreateAndValidateImportProof)
{
     ...
     * For the purposes of the test we will use one assetchain and a SAFE chain.
     *
    ...
    blocks.resize(1000);
    NotarisationData a2safe(0), safe2a(1);
    ...
    auto RunTestAssetchain = [&] ()
    {
        ...
        for (int ni=0; ni<numTestNotarisations; ni++)
        {
            ...
            n.MoM = endianHash(safecoin_calcMoM(n.height=height, n.MoMDepth=2));
    ...
    const char endpoint[] = "ipc://tmpSafecoinTestCrossChainSock";
    ...
    if (!childPid) {
        ...
        unlink("tmpSafecoinTestCrossChainSock");
