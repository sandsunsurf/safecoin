...
// for l in `g 'parse notarisation' ~/.safecoin/debug.log | pyline 'l.split()[8]'`; do hoek decodeTx '{"hex":"'`src/safecoin-cli getrawtransaction "$l"`'"}' | jq '.outputs[1].script.op_return' | pyline 'import base64; print base64.b64decode(l).encode("hex")'; done
