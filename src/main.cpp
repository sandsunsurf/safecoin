...
extern int32_t SAFECOIN_LOADINGBLOCKS,SAFECOIN_LONGESTCHAIN;
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,bool* pfMissingInputs, bool fRejectAbsurdFee)
{
    ...
    
    {
        ...
        CAmount nValueIn = 0;
        {
            ...
            if (!view.HaveJoinSplitRequirements(tx) and nextBlockHeight > 104300)  //sc
            {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck,bool fCheckPOW)
{
    ...
    if ( ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 && ASSETCHAINS_COMMISSION != 0 && block.vtx[0].vout.size() > 1 )
    {
        ...
        if ( (checktoshis= safecoin_commission((const CBlock *)&block)) != 0 )
        {
...
static bool ActivateBestChainStep(CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( SAFECOIN_REWIND != 0 )
    {
        ...
        fprintf(stderr,">>>>>>>>>>> rewind start ht.%d -> SAFECOIN_REWIND.%d\n",chainActive.Tip()->nHeight,SAFECOIN_REWIND);
        while ( SAFECOIN_REWIND > 0 && (tipindex= chainActive.Tip()) != 0 && tipindex->nHeight > SAFECOIN_REWIND )
        {
            fBlocksDisconnected = true;
...
int32_t safecoin_reverify_blockcheck(CValidationState& state,int32_t height,CBlockIndex *pindex)
{
    static int32_t oneshot;
    CBlockIndex *tipindex; int32_t rewindtarget;
    if ( SAFECOIN_REWIND != 0 )
        oneshot = SAFECOIN_REWIND;
    if ( oneshot == 0 && IsInitialBlockDownload() == 0 && (tipindex= chainActive.Tip()) != 0 )
    {
        ...
        if ( SAFECOIN_LONGESTCHAIN > height+200 && SAFECOIN_NEWBLOCKS == 0 )
        {
            if (  GetAdjustedTime() > tipindex->nTime+3600*2 )
            {
                fprintf(stderr,"possible fork: tip.%d longest.%d newblock.%d lag.%d blocktime.%u\n",tipindex->nHeight,SAFECOIN_LONGESTCHAIN,height,(int32_t)(GetAdjustedTime() - tipindex->nTime),tipindex->nTime);
                /*SAFECOIN_REWIND = tipindex->nHeight - 11;
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Check proof of work
    if ( (nHeight > 57731) && block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams))
    {
        ...
        if ( fsize > incr )
        {
            ...
            if ( ignore != 0 )
            {
                ...
                while ( fread(ignore,1,incr,file) == incr )
                    fprintf(stderr,".");
                free(ignore);
                //fprintf(stderr,"blk.%d loaded %ld bytes set fpos.%ld loading.%d\n",(int)pos.nFile,(long)ftell(file),(long)fpos,SAFECOIN_LOADINGBLOCKS);
            }
        }
