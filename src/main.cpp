...
extern int32_t SAFECOIN_LOADINGBLOCKS,SAFECOIN_LONGESTCHAIN,SAFECOIN_INSYNC,SAFECOIN_CONNECTING;
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if ( ASSETCHAINS_PRIVATE != 0 )
        {
            ...
            if ( (txout.nValue > 0 && iscoinbase == 0) || tx.GetJoinSplitValueOut() > 0 )
            {
                ...
                if ( safecoin_isnotaryvout(destaddr) == 0 )
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,bool* pfMissingInputs, bool fRejectAbsurdFee)
{
    ...
    
    {
        ...
        {
            flag = 1;
            SAFECOIN_CONNECTING = (1<<30) + (int32_t)chainActive.LastTip()->nHeight + 1;
        }
...
{
    const CChainParams& chainparams = Params();
    if ( SAFECOIN_STOPAT != 0 && pindex->nHeight > SAFECOIN_STOPAT )
        return(false);
...
//fprintf(stderr,"add %d vouts\n",(int32_t)tx.vout.size());
    ...
    if ( ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 && ASSETCHAINS_COMMISSION != 0 )
    {
        ...
        if ( (checktoshis= safecoin_commission((CBlock *)&block)) != 0 )
        {
    ...
    {
        if ( ASSETCHAINS_SYMBOL[0] != 0 || pindex->nHeight >= SAFECOIN_NOTARIES_HEIGHT1 || block.vtx[0].vout[0].nValue > blockReward )
        {
...
bool static DisconnectTip(CValidationState &state, bool fBare = false) {
    ...
    //if ( ASSETCHAINS_SYMBOL[0] != 0 || pindexDelete->nHeight > 1400000 )
    {
        ...
        safecoin_notarized_height(&prevMoMheight,&notarizedhash,&txid);
    ...
    if (!fBare)
    {
        ...
        for (int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if (tx.IsCoinBase() || ((i == (block.vtx.size() - 1)) && safecoin_isPoS((CBlock *)&block) != 0) || !AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL))
    ...
    {
        CTransaction &tx = block.vtx[i];
        if ( ASSETCHAINS_STAKED != 0 && (i == (block.vtx.size() - 1)) && safecoin_isPoS((CBlock *)&block) != 0 )
        {
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    SAFECOIN_CONNECTING = (int32_t)pindexNew->nHeight;
    // Get the current commitment tree
    ...
    if ( SAFECOIN_LONGESTCHAIN != 0 && pindexNew->nHeight >= SAFECOIN_LONGESTCHAIN )
        SAFECOIN_INSYNC = 1;
    else SAFECOIN_INSYNC = 0;
    //fprintf(stderr,"connect.%d insync.%d\n",(int32_t)pindexNew->nHeight,SAFECOIN_INSYNC);
    if ( ASSETCHAINS_SYMBOL[0] == 0 && SAFECOIN_INSYNC != 0 )
...
static bool ActivateBestChainStep(CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( SAFECOIN_REWIND != 0 )
    {
        ...
        fprintf(stderr,">>>>>>>>>>> rewind start ht.%d -> SAFECOIN_REWIND.%d\n",chainActive.LastTip()->nHeight,SAFECOIN_REWIND);
        while ( SAFECOIN_REWIND > 0 && (tipindex= chainActive.LastTip()) != 0 && tipindex->nHeight > SAFECOIN_REWIND )
        {
            fBlocksDisconnected = true;
    ...
    if ( ASSETCHAINS_CC != 0 ) // CC contracts might refer to transactions in the current block, from a CC spend within the same block and out of order
    {
        ...
        while ( 1 )
        {
            for (i=0; i<block.vtx.size(); i++)
            {
                ...
                if ( tx.IsCoinBase() != 0 )
                    continue;
                else if ( ASSETCHAINS_STAKED != 0 && (i == (block.vtx.size() - 1)) && safecoin_isPoS((CBlock *)&block) != 0 )
                    continue;
                Tx = tx;
...
bool ProcessNewBlock(bool from_miner,int32_t height,CValidationState &state, CNode* pfrom, CBlock* pblock, bool fForceProcessing, CDiskBlockPos *dbp)
{
    ...
    //fprintf(stderr,"ProcessBlock %d\n",(int32_t)chainActive.LastTip()->nHeight);
    {
        LOCK(cs_main);
        if ( chainActive.LastTip() != 0 )
            safecoin_currentheight_set(chainActive.LastTip()->nHeight);
        checked = CheckBlock(&futureblock,height!=0?height:safecoin_block2height(pblock),0,*pblock, state, verifier,0);
        bool fRequested = MarkBlockAsReceived(hash);
        fRequested |= fForceProcessing;
        if ( checked != 0 && safecoin_checkPOW(0,pblock,height) < 0 ) //from_miner && ASSETCHAINS_STAKED == 0
        {
            checked = 0;
        ...
        if ( 1 )
        {
            // without the safecoin_ensure call, it is quite possible to get a non-error but null pindex returned from AcceptBlockHeader. In a 2 node network, it will be a long time before that block is reprocessed. Even though restarting makes it rescan, it seems much better to keep the nodes in sync
            safecoin_ensure(pblock,hash);
        }
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(uint256, CBlockIndex*)& item, mapBlockIndex)
    {
        ...
        //safecoin_pindex_init(pindex,(int32_t)pindex->nHeight);
    }
