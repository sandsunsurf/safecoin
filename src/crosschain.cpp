        ...
        std::vector<uint256> &moms, uint256 &destNotarisationTxid)
{
     ...
     *
     *    safeHeight      notarisations-0      notarisations-1
     *                         *********************|
     *        > scan backwards >
     */
    ...
    if (safeHeight < 0 || safeHeight > chainActive.Height())
        return uint256();
    ...
    for (i=0; i<NOTARISATION_SCAN_LIMIT_BLOCKS; i++) {
        if (i > safeHeight) break;
        NotarisationsInBlock notarisations;
        uint256 blockHash = *chainActive[safeHeight-i]->phashBlock;
        ...
        const TxProof assetChainProof, int32_t offset)
{
     ...
     * backnotarisation for B (given by safeheight of notarisation), find the MoM within the MoMs for
     * that range, and finally extend the proof to lead to the MoMoM (proof root).
     */
    EvalRef eval;
    uint256 MoM = assetChainProof.second.Exec(txid);

    // Get a safe height for given notarisation Txid
    int safeHeight;
    {
        ...
        safeHeight = blockIdx.GetHeight();
    }

    // We now have a safeHeight of the notarisation from chain A. So we know that a MoM exists
    // at that height.
    ...
    uint256 MoMoM = CalculateProofRoot(targetSymbol, targetCCid, safeHeight, moms, targetChainNotarisationTxid);
    if (MoMoM.IsNull())
...
/* On assetchain */
bool GetNextBacknotarisation(uint256 safeNotarisationTxid, Notarisation &out)
{
    /*
     ...
     */
    Notarisation bn;
    if (!GetBackNotarisation(safeNotarisationTxid, bn))
        return false;
...
bool CheckMoMoM(uint256 safeNotarisationHash, uint256 momom)
{
     ...
     * backnotarisation and scan around the safeheight to see if the MoMoM is a match.
     * This is a sledgehammer approach...
     */

    Notarisation bn;
    if (!GetBackNotarisation(safeNotarisationHash, bn))
        return false;
