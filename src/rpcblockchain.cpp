...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("confirmations", safecoin_dpowconfs(blockindex->nHeight,confirmations)));
...
UniValue kvsearch(const UniValue& params, bool fHelp)
{
    ...
    if ( (keylen= (int32_t)strlen(params[0].get_str().c_str())) > 0 )
    {
        ...
        if ( keylen < sizeof(key) )
        {
            ...
            if ( (valuesize= safecoin_kvsearch(&refpubkey,chainActive.LastTip()->nHeight,&flags,&height,value,key,keylen)) >= 0 )
            {
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    ...
    {
        ret.push_back(Pair("confirmations", safecoin_dpowconfs(coins.nHeight,pindex->nHeight - coins.nHeight + 1)));
    ...
    if ( (interest= safecoin_accrued_interest(&txheight,&locktime,hash,n,coins.nHeight,coins.vout[n].nValue,(int32_t)pindex->nHeight)) != 0 )
