...
cJSON *get_gatewayspending(char *refcoin,char *acname,char *oraclestxidstr)
{
    ...
    if ( (retjson= get_safecoincli(refcoin,&retstr,acname,"gatewayspending",oraclestxidstr,refcoin,"","")) != 0 )
    {
        //printf("pending.(%s)\n",jprint(retjson,0));
        return(retjson);
    }
...
char *get_gatewaysmultisig(char *refcoin,char *acname,char *bindtxidstr,char *withtxidstr,char *txidaddr)
{
    ...
    if ( (retjson= get_safecoincli("SAFE",&retstr,acname,"gatewaysmultisig",bindtxidstr,refcoin,withtxidstr,txidaddr)) != 0 )
    {
...
void gatewaysmarkdone(char *refcoin,char *acname,bits256 withtxid,char *coin,bits256 cointxid)
{
    ...
    if ( (retjson= get_safecoincli(refcoin,&retstr,acname,"gatewaysmarkdone",bits256_str(str,withtxid),coin,bits256_str(str2,cointxid),"")) != 0 )
    {
        ...
        if ( jint(retjson,"queueflag") != 0 && (coinstr= jstr(retjson,"coin")) != 0 && strcmp(coinstr,refcoin) == 0 )
        {
            if ( (pending= jarray(&n,retjson,"pending")) != 0 )
            {
                for (i=0; i<n; i++)
                {
                    ...
                    {
                        if ( (satoshis= jdouble(item,"amount")*SATOSHIDEN) != 0 && (retval= coinaddrexists("SAFE",acname,txidaddr)) == 0 )
                        {
                            // this is less errors but more expensive: ./safecoin-cli z_sendmany "signeraddr" '[{"address":"<txidaddr>","amount":0.0001},{"address":"<withdrawaddr>","amount":<withamount>}]'
                            txid = sendtoaddress("SAFE",acname,txidaddr,10000);
                            if ( bits256_nonz(txid) != 0 && coinaddrexists("SAFE",acname,txidaddr) > 0 )
                            {
                                // the actual withdraw
                                if ( strcmp(depositaddr,signeraddr) == 0 )
                                {
                                    ...
                                    if ( bits256_nonz(cointxid) != 0 )
                                    {
                                        ...
                                        gatewaysmarkdone("SAFE",acname,origtxid,refcoin,cointxid);
                                        processed++;
                                    }
                                    else
                                    {
                                ...
                                else
                                {
                                    ...
                                    if ( rawtx != 0 )
                                    {
                                        if ( (clijson= addmultisignature(refcoin,"",signeraddr,rawtx)) != 0 )
                                        {
                                            ...
                                            {
                                                cointxid = safecoinbroadcast(refcoin,"",clijson);
                                                if ( bits256_nonz(cointxid) != 0 )
                                                {
                                            ...
                                            {
                                                // 10000 + ith -> txidaddr
                                                txid = safecoinbroadcast("SAFE",acname,clijson);
                        ...
                        else if ( retval > 0 )
                        {
                            ...
                            gatewaysmarkdone("SAFE",acname,origtxid,refcoin,zeroid);
                        }
                    }
                }
            }
    ...
    while ( 1 )
    {
        ...
        if ( (refcoin[0] == 0 || prevheight < (get_coinheight(refcoin,"") - 10)) && (clijson= get_safecoincli("SAFE",&retstr,acname,"oraclesinfo",oraclestr,"","","")) != 0 )
        {
            if ( refcoin[0] == 0 && jstr(clijson,"name") != 0 )
            {
                ...
                if ( get_gatewaysinfo("SAFE",acname,depositaddr,&M,&N,bindtxidstr,refcoin,oraclestr) < 0 )
                {
