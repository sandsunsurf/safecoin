...
int32_t rogue_sendrawtransaction(char *rawtx)
{
    ...
    if ( (retstr= safecoin_issuemethod(USERPASS,"sendrawtransaction",params,ROGUE_PORT)) != 0 )
    {
...
int32_t rogue_progress(struct rogue_state *rs,int32_t waitflag,uint64_t seed,char *keystrokes,int32_t num)
{
    ...
    if ( rs->guiflag != 0 && Gametxidstr[0] != 0 )
    {
        ...
        if ( 0 && (pastkeys= rogue_keystrokesload(&numpastkeys,seed,1)) != 0 )
        {
            ...
            if ( (retstr= safecoin_issuemethod(USERPASS,"cclib",params,ROGUE_PORT)) != 0 )
            {
                if ( (retjson= cJSON_Parse(retstr)) != 0 )
                {
        ...
        if ( 0 )
        {
            sprintf(cmd,"./safecoin-cli -ac_name=ROGUE cclib keystrokes 17 \\\"[%%22%s%%22,%%22%s%%22]\\\" >> keystrokes.log",Gametxidstr,hexstr);
            if ( system(cmd) != 0 )
...
int32_t rogue_setplayerdata(struct rogue_state *rs,char *gametxidstr)
{
    ...
    if ( 0 )
    {
        ...
        sprintf(cmd,"./safecoin-cli -ac_name=ROGUE cclib gameinfo 17 \\\"[%%22%s%%22]\\\" > %s",gametxidstr,fname);
        if ( system(cmd) != 0 )
            fprintf(stderr,"error issuing (%s)\n",cmd);
    ...
    else
    {
        ...
        filestr = safecoin_issuemethod(USERPASS,"cclib",params,ROGUE_PORT);
    }
    if ( filestr != 0 )
    {
...
int main(int argc, char **argv, char **envp)
{
    ...
    ROGUE_PORT = safecoin_userpass(userpass,ASSETCHAINS_SYMBOL);
    if ( IPADDRESS[0] == 0 )
    ...
    else
    {
        ...
        if ( ROGUE_PORT == 0 )
        {
            printf("you must copy ROGUE.conf from ~/.safecoin/ROGUE/ROGUE.conf (or equivalent location) to current dir\n");
            return(-1);
        }
