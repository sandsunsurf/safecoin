...
void safenodes_initQ(queue_t *Q,char *name)
{
...
void safecoin_args(char *argv0)
{
    ...
    if ( name.c_str()[0] != 0 )
    {
        ...
        if ( ASSETCHAINS_ENDSUBSIDY != 0 || ASSETCHAINS_REWARD != 0 || ASSETCHAINS_HALVING != 0 || ASSETCHAINS_DECAY != 0 || ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_PUBLIC != 0 || ASSETCHAINS_PRIVATE != 0 )
        {
            ...
            extralen += safenodes_rwnum(1,&extraptr[extralen],sizeof(ASSETCHAINS_ENDSUBSIDY),(void *)&ASSETCHAINS_ENDSUBSIDY);
            extralen += safenodes_rwnum(1,&extraptr[extralen],sizeof(ASSETCHAINS_REWARD),(void *)&ASSETCHAINS_REWARD);
            extralen += safenodes_rwnum(1,&extraptr[extralen],sizeof(ASSETCHAINS_HALVING),(void *)&ASSETCHAINS_HALVING);
            extralen += safenodes_rwnum(1,&extraptr[extralen],sizeof(ASSETCHAINS_DECAY),(void *)&ASSETCHAINS_DECAY);
            ...
            extralen += safenodes_rwnum(1,&extraptr[extralen],sizeof(val),(void *)&val);
        }
        ...
        if ( (baseid= safecoin_baseid(ASSETCHAINS_SYMBOL)) >= 0 && baseid < 32 )
            MAX_MONEY = safecoin_maxallowed(baseid);
        ...
        safenodes_rwnum(1,magic,sizeof(ASSETCHAINS_MAGIC),(void *)&ASSETCHAINS_MAGIC);
        for (i=0; i<4; i++)
        ...
        if ( (fp= fopen(fname,"wb")) != 0 )
        {
            fprintf(fp,safenodesfmtstr,name.c_str(),name.c_str(),name.c_str(),name.c_str(),magicstr,ASSETCHAINS_P2PPORT,ASSETCHAINS_RPCPORT,"78.47.196.146");
            fclose(fp);
            //printf("created (%s)\n",fname);
        } else printf("error creating (%s)\n",fname);
#endif
        if ( SAFECOIN_CCACTIVATE != 0 && ASSETCHAINS_CC < 2 )
        {
            ASSETCHAINS_CC = 2;
