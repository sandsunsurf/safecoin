...
uint16_t safecoin_userpass(char *username,char *password,FILE *fp)
{
...
void safecoin_configfile(char *symbol,uint16_t port)
{
    ...
    if ( symbol != 0 && port != 0 )
    {
        ...
        {
            safecoin_userpass(myusername,mypassword,fp);
    ...
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        if ( (safeport= safecoin_userpass(username,password,fp)) != 0 )
            SAFE_PORT = safeport;
...
uint16_t safecoin_userpass(char *userpass,char *symbol)
{
    ...
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        port = safecoin_userpass(username,password,fp);
...
void safecoin_args(char *argv0)
{
    ...
    ASSETCHAINS_CC = GetArg("-ac_cc",0); // keep it outside the assetchains hashing so SAFE can do it and we dont have two identical chains other than -ac_cc
    ...
    if ( name.c_str()[0] != 0 )
    {
        ...
        ASSETCHAINS_PORT = safecoin_port(ASSETCHAINS_SYMBOL,ASSETCHAINS_SUPPLY,&ASSETCHAINS_MAGIC,extraptr,extralen);
        ...
        if ( ASSETCHAINS_SYMBOL[0] != 0 )
        {
            ...
            safecoin_configfile(ASSETCHAINS_SYMBOL,ASSETCHAINS_PORT + 1);
            safecoin_userpass(ASSETCHAINS_USERPASS,ASSETCHAINS_SYMBOL);
            COINBASE_MATURITY = 1;
    ...
    else
    {
        ...
        for (iter=0; iter<2; iter++)
        {
            ...
            if ( (fp= fopen(fname,"rb")) != 0 )
            {
                safecoin_userpass(username,password,fp);
