...
//#endif
    ...
    obj.push_back(Pair("errors",        GetWarnings("statusbar")));
    {
        ...
        if ( (notaryid= safecoin_whoami(pubkeystr,(int32_t)chainActive.LastTip()->nHeight,safecoin_chainactive_timestamp())) >= 0 )
        {
...
UniValue coinsupply(const UniValue& params, bool fHelp)
{
    ...
    if (height >= 0 && height <= currentHeight) {
        if ( (supply= safecoin_coinsupply(&zfunds,height)) > 0 )
        {
