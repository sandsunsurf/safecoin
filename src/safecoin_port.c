...
uint16_t safecoin_calcport(char *name,uint64_t supply,uint64_t endsubsidy,uint64_t reward,uint64_t halving,uint64_t decay)
{
...
int main(int argc, char * argv[])
{
    ...
    rpcport = 1 + safecoin_calcport(argv[offset + 1],supply,endsubsidy,reward,halving,decay);
    printf("./safecoind -ac_name=%s -ac_supply=%llu -ac_end=%llu -ac_reward=%llu -ac_halving=%llu -ac_decay=%llu & # rpcport %u\n",argv[offset + 1],(long long)supply,(long long)endsubsidy,(long long)reward,(long long)halving,(long long)decay,rpcport);
    if ( allocated != 0 )
    {
        ...
        for (i=0; i<num; i++)
        {
            ...
            while ( 1 )
            {
                rpcport = 1 + safecoin_calcport(newname,supply+j,endsubsidy,reward,halving,decay);
