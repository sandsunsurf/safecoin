...
Contents:
safecoind.exe - VerusCoin's enhanced Safecoin daemon
safecoin-cli.exe - VerusCoin's Safecoin command line utility
verus.bat - wrapper for safecoin-cli that applies the command to the VRSC coin
verusd.bat - wrapper for safecoind that sets the VerusCoin parameters to defaults properly
...
The first time on a new system you will need to run fetch-params before using safecoind.exe or verusd.
...
Extreme cases can result in the virus scanner deleting Safewallet.exe or moving it to "protect" the system. You will to add the executables to a whitelist and re-extract the verus-cli-windows.zip file if that happens.
Run verusd.bat to launch safecoind, and use verus.bat to run commands such as:
verus.bat stop
Which signals safecoind.exe (if it is running) to stop running.
