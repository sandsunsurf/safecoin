...
during the *configure* step of building safecoind:
...
Launch safecoind like this:

    $ safecoind -amqppubhashtx=amqp://127.0.0.1:5672
...
These options can also be provided in safecoin.conf.
...
From the perspective of safecoind, the local end of an AMQP link is write-only.

No information is broadcast that wasn't already received from the public
P2P network.

No authentication or authorization is done on peers that safecoind connects
...
At present, safecoind does not try to resend a notification if there was
...
Currently, safecoind appends an up-counting sequence number to each notification
